; computational-complexity.scm
;; CSC-151-NN (fall)
;; Computational complexity (computational-complexity.scm)
;; Authors: Sarah Kim, Vy Nguyen, A'aisha Azam
;; Date: 12/08/2025
;; Acknowledgements:
;;   professor, mentor
...
; (|> (range 16) alphabetically-first-1 rletters)
; (define letters (|> (range 15)
...
(define list-of-strings
  (map (section list-take rletters _) (map (section + _ 1) (range 16))))
...
(define alph-1
  (map (section counter-helper _ alphabetically-first-1) list-of-strings))

(define alph-2
  (map (section counter-helper _ alphabetically-first-2) list-of-strings))
alph-1
alph-2
...
(define data-alph-1
  (map (section pair _ _) (map (section + _ 1) (range 16)) alph-1))

(define data-alph-2
  (map (section pair _ _) (map (section + _ 1) (range 16)) alph-2))

(plot-linear
  (dataset-line "alphabetically-first-1" data-alph-1)
   (dataset-line "alphabetically-first-2" data-alph-2))


; TODO: Write code to plot the results
; If we results1 is the result of running alphabetically-first-1
; in the following format: 
;      (list (pair 1 1) (pair 2 3) (pair 3 7) ....)
; and likewise for results2, we can use a plot to compare the
; two as below

;(plot-linear
;  (dataset-line "alphabetically-first-1"
;    results1)
;  (dataset-line "alphabetically-first-2"
;    results2))
...
(define list-reversing
  (map (section list-take rletters _) (map (section + _ 1) (range 16))))


(define list-counting
  (map (section counter-helper _ alphabetically-first-1) list-of-strings)) 
