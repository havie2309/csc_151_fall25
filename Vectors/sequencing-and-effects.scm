...
;;
;; Vector-set change the value of x and it is hard to mentally trace the computation. 
;; The "z" would change based on the vector-set of last line, which is "ignore (vector-set! x 0 2)"
...
; (a)
(f (vector "q" "o" "l" "p" "e"))
 ; --> {{ vec = (vector "q" "o" "l" "p" "e")}}
 ; (begin
 ;      (vector-set! (vector "q" "o" "l" "p" "e") 0 "h")
 ;      (vector-set! (vector "q" "o" "l" "p" "e") 3 (vector-ref (vector "q" "o" "l" "p" "e") 2))
 ;      (let ([x (vector-ref (vector "q" "o" "l" "p" "e") 1)])
 ;        (begin
 ;          (vector-set! vec 1 (vector-ref vec 4))
 ;          (vector-set! vec 4 x)
 ;          vec)))))

 ; --> {{ vec = (vector "h" "o" "l" "p" "e")}}
 ; (begin
 ;      (vector-set! (vector "h" "o" "l" "p" "e") 3 (vector-ref (vector "h" "o" "l" "p" "e") 2))
 ;      (let ([x (vector-ref (vector "h" "o" "l" "p" "e") 1)])
 ;        (begin
 ;          (vector-set! vec 1 (vector-ref vec 4))
 ;          (vector-set! vec 4 x)
 ;          vec))))

 ; --> {{ vec = (vector "h" "o" "l" "l" "e")}}
 ; (begin
 ;      (vector-set! (vector "h" "o" "l" "p" "e") 3 "l"))
 ;      (let ([x (vector-ref (vector "h" "o" "l" "p" "e") 1)])
 ;        (begin
 ;          (vector-set! vec 1 (vector-ref vec 4))
 ;          (vector-set! vec 4 x)
 ...
 ;          x = "o"  }}
 ;        (begin
 ;          (vector-set! (vector "h" "o" "l" "l" "e" 1 (vector-ref (vector "h" "o" "l" "l" "e" 4))
 ;          (vector-set! vec 4 x)
 ;          vec))))
 ...
 ;          x = "o"  }}
 ;        (begin
 ;          (vector-set! (vector "h" "o" "l" "l" "e" 1 "e"))
 ;          (vector-set! vec 4 x)
 ;          vec))))

 ; --> {{ vec = (vector "h" "e" "l" "l" "e")
 ;          x = "o"  }}
 ;        (begin
 ;          (vector-set! vec 4 x)
 ;          vec))))
...
(g (vector 5 3))
; {{ vec = (vector 5 3)}}
;     (if (zero? (vector-ref (vector 5 3) 1))
;         (vector-ref vec 0)
;         (begin
;           (vector-set! vec 0 (* 2 (vector-ref vec 0)))
;           (vector-set! vec 1 (- (vector-ref vec 1) 1))
...
; {{ vec = (vector 5 3)}}
;         (begin
;           (vector-set! (vector 5 3) 0 (* 2 (vector-ref (vector 5 3) 0)))
;           (vector-set! vec 1 (- (vector-ref vec 1) 1))
;           (g vec)))))

; {{ vec = (vector 5 3)}}
;         (begin
;           (vector-set! (vector 5 3) 0 (* 2 5))
;           (vector-set! vec 1 (- (vector-ref vec 1) 1))
;           (g vec)))))

; {{ vec = (vector 5 3)}}
;         (begin
;           (vector-set! (vector 5 3) 0 10)
;           (vector-set! vec 1 (- (vector-ref vec 1) 1))
;           (g vec)))))

; {{ vec = (vector 10 3)}}
;         (begin
;           (vector-set! (vector 10 3) 1 (- (vector-ref (vector 10 3) 1) 1))
;           (g vec)))))

; {{ vec = (vector 10 3)}}
;         (begin
;           (vector-set! (vector 10 3) 1 (- 3 1))
;           (g vec)))))
...
; {{ vec = (vector 20 1)}}
;     (if (zero? (vector-ref (vector 20 1) 1))
;         (vector-ref vec 0)
;         (begin
;           (vector-set! vec 0 (* 2 (vector-ref vec 0)))
;           (vector-set! vec 1 (- (vector-ref vec 1) 1))
  ...
  (lambda (vec val n)
    (if (negative? n)
      ...
      (begin 
        ...
        (my-vector-fill!-helper vec val (- n 1))))))

(define my-vector-fill!
  (lambda (vec val)
    (my-vector-fill!-helper vec val (vector-length vec))))
...
(define vector-reverse-helper
  (lambda (vec n)
    (if (> n 0) 
      vec
      (let ([temp-vec vec])
        (begin
        (vector-set! vec n (vector-ref temp-vec (- (vector-length vec) n)))
        (vector-reverse-helper vec (- n 1)))))))
    
(define vector-reverse
  (lambda (vec)
    (vector-reverse-helper vec (- (vector-length vec) 1))))
