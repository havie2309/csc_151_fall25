...
;; Authors: Hoang Nguyen, Vy Nguyen
;; Date: 10/29/2025
;; Acknowledgements:
...
;; called digits->nums. (digits->nums lst) takes a list of characters as
...
;;
;; (digits->nums (list #\a #\0 #\q #\!))
;; > (list 97 48 113 33)
;; (digits->nums null)
;; > null

;;; (digits->nums lst) -> list?
;;; lst: list?
;;; Returns a list where each character has been turned into its
;; integer codepoint value, i.e., its collating sequence number.
(define digits->nums
  (lambda (lst)
    (match lst
      [null null]
      [(cons head tail) (cons (char->integer head) (digits->nums tail))])))

(test-case "normal test cases"
  equal? (map char->integer (list #\a #\0 #\q #\!))
    (lambda () (digits->nums (list #\a #\0 #\q #\!))))

(test-case "all numeric chars"
  equal? (map char->integer (list #\1 #\0 #\2 #\3))
    (lambda () (digits->nums (list #\1 #\0 #\2 #\3))))
...
(description "Shared between functions: the base cases always return null
                                        the recursive case follows the pattern of 
                                        (cons (fun head) (f tail)")

(description "Differences between functions: they serve different purposes")
...
;;; (list-map f l) -> list?
;;;   f: procedure? (take one parameter)
;;;   l: list?
;;; Returns null if list is null, while returns (cons (f head) (list-map f tail)) when otherwise
(define list-map
  (lambda (f l)
    (match l
      [null null]
...
(test-case "square"
  equal? (map square (list 1 2 3 4 5))
    (lambda () (list-map square (list 1 2 3 4 5))))

(test-case "even?"
  equal? (map even? (list 1 2 3 4 5))
    (lambda () (list-map even? (list 1 2 3 4 5))))

(test-case "divisible by 5"
  equal? (map (section = 0 (remainder _ 5)) (list 1 2 3 4 5))
    (lambda () (list-map (section = 0 (remainder _ 5)) (list 1 2 3 4 5))))
...
;;; (dropfalses lst) -> list?
;;;  lst: list?
;;; Returns a new list with all the #f values removed from the lst
(define dropfalses
  (lambda (lst)
    (match lst
      [null null]
...
(test-case "normal list"
  equal? (filter (section equal? _ #t) (list #t #f #t #f #f))
    (lambda () (dropfalses (list #t #f #t #f #f))))

(test-case "all false list"
  equal? (filter (section equal? _ #t) (list #f #f #f #f #f))
    (lambda () (dropfalses (list #f #f #f #f #f))))

(test-case "all true list"
  equal? (filter (section equal? _ #t) (list #t #t #t #t #t))
    (lambda () (dropfalses (list #t #t #t #t #t))))

(test-case "empty list"
  equal? (filter (section equal? _ #t) (list ))
    (lambda () (dropfalses (list ))))

;; Like the previous problem, first identify what is shared and different
;; between these three functions:

(description "Shared between functions: the base cases always return null
                                        the recursive case always check the opposite conditions: 
                                        if #t then skip the head element, otherwise 
                                        add the first element 
  onto the recursive calls of the rest of the list")
...
;;; (list-filter pred? l) -> list?
;;; pred?: predicate?, accepts one parameter
;;; l: list?
;;; Returns a list whose elements satisfy pred? in l.
(define list-filter
  (lambda (pred? l)
    (match l
      [null null]
...
(test-case "drop zeros"
  equal? (filter (section = _ 0) (list 1 0 0 0 0 0 1))
    (lambda () (list-filter (section = _ 0) (list 1 0 0 0 0 0 1))))

(test-case "drop false"
  equal? (filter (section equal? _ #f) (list #t #f #t #f #f))
    (lambda () (list-filter (section equal? _ #f) (list #t #f #t #f #f))))
...
;;; (string-append-backwards result l) -> string?
;;;  result: string?
;;;  l: list of string?
;;; Returns the strings of the list appended onto the front of the initial string in backwards order.
(define string-append-backwards
  (lambda (result l)
    (match l
      [null result]
      [(cons head tail) (string-append-backwards (string-append head result) tail)])))

(test-case "normal case"
  equal? "gihdefabc"
    (lambda () (string-append-backwards "abc" (list "def" "h" "gi"))))

(test-case "empty list case"
  equal? "abc"
    (lambda () (string-append-backwards "abc" (list ))))


;; Like the previous problems, first identify what is shared and different
;; between these three functions:

(description "Shared between functions: the base cases always return the current result
                                        the recursive case always call the procedure with the 
                                        updated result and tail")

(description "Differences between functions: they serve the different purposes")
...
;;; (list-fold-left f v l) -> same type of elements with v
;;; f: procedure?, taking 2 parameters
;;; v: initial value, any type?
;;; l: list?
;;; Simulate fold by updating the initial result by calling f
;;; with the top element of the list and the initial value
(define list-fold-left
  (lambda (f v l)
    (match l
      [null v]
      [(cons head tail) (list-fold-left f (f v head) tail)])))

(test-case "string-append"
  equal? (fold string-append "abc" (list "def" "h" "gi"))
    (lambda () (list-fold-left string-append "abc" (list "def" "h" "gi"))))

(test-case "+"
  equal? (fold + 0 (list 1 2 3))
    (lambda () (list-fold-left + 0 (list 1 2 3))))

(test-case "/"
  equal? (fold / 1 (list 1 2 3))
    (lambda () (list-fold-left / 1 (list 1 2 3))))
...
; In the lab, you were tasked with implementing
; fold. You might have noticed two possible
; implementations:
