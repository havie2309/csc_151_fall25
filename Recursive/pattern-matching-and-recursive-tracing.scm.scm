...
;; Authors: Patric Desir, Vy Nguyen
...
; --> (cdr (car (cdr (cons 1 (cons 2 (list 3))))))
; (cdr (car (cdr (cons 1 (list 2 3)))))
; (cdr (car (cdr (list 1 2 3))))
; (cdr (car (list 2 3)))
; (cdr 2)
; error
...
; --> (list->string (cdr (cdr (cdr (list #\h #\e #\l #\l #\o #\space #\w #\o #\r #\l #\d #\!)))))
; (list->string (cdr (cdr (list #\e #\l #\l #\o #\space #\w #\o #\r #\l #\d #\!))))
; (list->string (cdr (list #\l #\l #\o #\space #\w #\o #\r #\l #\d #\!)))
; (list->string (list #\l #\o #\space #\w #\o #\r #\l #\d #\!))
; "lo world!"
...
;     (lambda (l) (fold (lambda (s c) (cons c s)) null l))
;     list->string)
; --> (|> (list #\! #\d #\l #\r #\o #\w #\space #\o #\l #\l #\e #\h)
;     list->string)
; --> "!dlrow olleh"
...
; --> (match (list 1 8 2)
;      [null (list 9)]
;      [(cons head tail) (cons head (func-4 9 tail))])
; --> (cons 1 (func-4 9 (list 8 2)))
; --> (cons 1 (cons 8 (func-4 9 (list 2))))
; --> (cons 1 (cons 8 (cons 2 (func-4 9 null))))
; --> (cons 1 (cons 8 (cons 2 (list 9))))
; --> (list 1 8 2 9)

; c. Explanation

(description "Base case: If l is null, appends x to l")

(description "Recursive case: If l is not null, appends the head to (func-4 x tail)")

(description "As a whole, func-4 will add x to the end of list l")
...
(func-5 5 (list 1 9 4 8))

(func-5 5 (list 9 1 9 4 4 8 7 1))

(func-5 9 (list 9 1 9 4 4 8 7 1))

(func-5 1 (list 9 1 9 4 4 8 7 1))

; b. Trace
(func-5 7 (list 2 1 7 2 7))
; --> ...
; --> (match (list 2 1 7 2 7)
      ; [null null]
      ; [(cons head tail)
      ;  (if (equal? head 7)
      ;      (func-5 7 tail)
      ;      (cons head (func-5 7 tail)))])
      ; (if (equal? 2 7)
      ;      (func-5 7 (list 1 7 2 7))
      ;      (cons 2 (func-5 7 (list 1 7 2 7))))
      ; (cons 2 (func-5 7 (list 1 7 2 7)))
; --> (cons 2 (func-5 7 (list 1 7 2 7)))
;(cons 2 (match (list 1 7 2 7)
      ; [(cons head tail)
      ;  (if (equal? 1 2)
      ;      (func-5 2 (list 7 2 7))
      ;      (cons 1 (func-5 7 (list 7 2 7))))])
      ; (cons 2 (cons 1 (func-5 7 (list 7 2 7))))
;(cons 2 (cons 1 (match (list 7 2 7)
      ; [(cons head tail)
      ;  (if (equal? 7 7)
      ;      (func-5 7 (list 2 7))
      ;      (cons 2 (cons 1 (func-5 7 (list 2 7))))])
      ; (cons 2 (cons 1 (func-5 7 (list 2 7))))
;(cons 2 (cons 1 (match (list 2 7)
      ; [(cons head tail)
      ;  (if (equal? 2 7)
      ;      (func-5 7 (list 7))
      ;      (cons 2 (cons 1 (func-5 7 (list 7))))])
      ; (cons 2 (cons 1 (cons 2 (func-5 7 (list 7)))))
;(cons 2 (cons 1 (cons 2 (match (list 7)
      ; [(cons head tail)
      ;  (if (equal? 7 7)
      ;      (func-5 7 null)
      ;      (cons 2 (cons 1 (cons 2 (func-5 7 (list 7))))])
      ; (cons 2 (cons 1 (cons 2 (func-5 7 null)))
;(cons 2 (cons 1 (cons 2 (func-5 7 null)))
; (list 2 1 2)

; c. Explanation

(description "Base case: If the list is null, it will give back null")

(description "Recursive case: If the list is not null, it will remove the x out of the list")
...
(description "Recursion \"works\" because there is a base case that doesn't need to call recursion. 
  So the function has the stop point and it will not repeat itself infinitively.")
