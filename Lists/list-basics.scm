...
(description "(list 2 1) --> (2 1)")
; (display (list 2 1))

(description "(make-list 1 2) --> (2)")
 ;(display (make-list 1 2))

(description "(make-list -1 2) --> ()")
 ;(display (make-list -1 2))

(description "(range 3) --> (0 1 2)")
; (display (range 3))

(description "(map range (list 2 1)) --> (0 1 0)")
...
(description "(range 5 10) --> (5 6 7 8 9)")
; (display (range 5 10))

(description "(range -4 3) --> (-4 -3 -2 -1 0 1 2)")
; (display (range -4 3))

(description "(apply + (range 10)) --> 45")
...
(description "(apply + (range 1 10 2)) --> 25")
...
(description "`range` has so many different inputs because in order to understand
  how it works, we must test a variety of different inputs and number of inputs;
  it's also because it works in a few very specific ways.")
...
(description
  "Other procedures include string->list, string-split. ")
...
(description "To convert a string into an integer, turn every character
  in the string into its corresponding integer value, then multiply each
  of those values by their corresponding place in the number (ex. 1 x 100,
  2 x 10) and then add all those values up.")
...
(test-case "(char->digit #\\4)"
  equal? 4
  (lambda () (char->digit #\4)))


(test-case "(char->digit #\\7)"
  equal? 7
  (lambda () (char->digit #\7)))
...
(test-case "(string->digits \"12303\")"
  equal? (list 1 2 3 0 3)
  (lambda () (string->digits "12303")))

(test-case "(string->digits \"458\")"
  equal? (list 4 5 8)
  (lambda () (string->digits "458")))
...
(description "To convert a string into an integer with the tools we have,
  use string->digits to return a list of digits. Then use reverse on
  powers-of-ten with the length of the string as the input for powers-
  of-ten. Finally, map the * procedure to both lists and apply the + 
  procedure around the whole thing.")
...
(define string->integer
  (lambda (str)
    (apply + (map * (string->digits str) (reverse (powers-of-ten (string-length str)))))))
...
(define check-letter
  (lambda (str)
    (integer->char (+ 65 (remainder (apply + (map char->integer (string->list str))) 26)))))

(test-case "(check-letter (\"Hello\"))"
  equal? #\G
  (lambda () (check-letter "Hello")))
