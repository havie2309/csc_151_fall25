...
; that your final value coincides with the
...
; `f` and `g`?
;
; They both do the same purpose of comparing 2 numbers, return #t if x1 is smaller than x2, #f otherwise. 
; procedure g is more compact and efficient than procedure f.
...
; "3 2 4" and "2 3 1")?
;
; Because we want to have different orders of inputs, so that we can have different outputs in the end. 

; d. What do you see as the similarities and differences between
; `baz` and `qux`?
;
; They both do the same purpose of comparing 3 numbers, return #t if they are in ascending order, #f otherwise.
; Procedure qux is more compact and efficient than procedure baz.
...
;
; When you are done, you should experiment with your function
; on a variety of inputs to verify that the program works as expected.
...
; b. In your experiments with `is-even?`, you may have deter mined that
...
(define better-both-even?
  (lambda (n1 n2)
    (and (is-even? n1) (is-even? n2))))

(better-both-even? 2 5)
...
;
; (<expr>)
; Since <expr> is a boolean statement, it would return true or false depending on its evaluation,
; without needing the if statement.
...
(define employee-id? 
  (lambda (str)
    (and (= (string-length str) 6) (integer? (substring str 0 1))
      (integer? (substring str 1 2)) (integer? (substring str 2 3)) (integer? (substring str 3 4)) (integer? (substring str 4 5))
      (integer? (substring str 5 6)))))
...
(define founder-id? 
  (lambda (sstr)
    (and (employee-id? sstr) (string=? (substring sstr 0 3) "000"))))
...
; c. Work on enhancing this checker so that it accepts other reasonable forms of "Yes",
...
; inputs to ensure that the function works.
